...
public class FriendResourceIntTest {
    ...
    public void getAllFriendsWithEagerRelationshipsIsEnabled() throws Exception {
        FriendResource friendResource = new FriendResource(friendServiceMock);
        when(friendServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restFriendMockMvc = MockMvcBuilders.standaloneSetup(friendResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
        ...
        verify(friendServiceMock, times(1)).findAllWithEagerRelationships(any());
    }
