...
public class FriendResource {
     ...
     */
    @GetMapping("/friends")
    public ResponseEntity<List<FriendDTO>> getAllFriends(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Friends");
        Page<FriendDTO> page;
        if (eagerload) {
            page = friendService.findAllWithEagerRelationships(pageable);
        } else {
            page = friendService.findAll(pageable);
        }
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, String.format("/api/friends?eagerload=%b", eagerload));
